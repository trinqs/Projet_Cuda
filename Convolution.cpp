                ...
                for( int i=0; i<3; i++){
                    ...
                    for (int decalageCol = -limCols; decalageCol < limCols+1; decalageCol++){
                        for (int decalageRow = -limRows; decalageRow < limRows+1; decalageRow++){
                            ...
                            sum += rgb[3*(( row + decalageCol )*imgCols+( col + decalageRow ))+i] * noyau.matrice[ decalageRow + limRows ][ decalageCol + limCols ];//coefficient de la matrice de convolution à l'indice associé, on fait la rotation en même temps par le calcul d'indice
                            ...
                            cout << "coordonnees de la matrice (" << decalageRow + limRows << "," << decalageCol + limCols << ")" <<endl;
                        }
                    }
